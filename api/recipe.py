...
from models.recipe import db, Recipe, recipe_schema
...
def add_recipe(current_user):
    ...
    for ingredient in request.json.get("ingredients", []):
        ...
        recipe_ingredient = RecipeIngredient(
            recipe=new_recipe, ingredient_id=ingredient_id, quantity=quantity
        )
...
def get_all_recipes(current_user):
    ...
    for recipe in all_recipes:
        ...
        recipe_data["ingredients"] = [
            recipe_ingredient_schema.dump(ingredient)
...
def get_recipe(current_user, recipe_id):
    ...
    recipe_data = recipe_schema.dump(recipe)
    recipe_data["ingredients"] = [
        recipe_ingredient_schema.dump(ingredient) for ingredient in recipe.ingredients
    ]
...
def update_recipe(current_user, recipe_id):
    ...
    for ingredient in request.json.get("ingredients", []):
        ...
        quantity = ingredient["quantity"]
        recipe_ingredient = RecipeIngredient(
            recipe=recipe, ingredient_id=ingredient_id, quantity=quantity
        )
        db.session.add(recipe_ingredient)
